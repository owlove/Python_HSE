...
(Разобранный пример с пары с корректировками)
"""
...
def calcNPV(firstCost, income, cost, implTime, disRate):
    ...
    try:
        ...
        print("Некорректные входные данные для подсчета NPV, DPP")
    return NPV, DPP
...
try:
    calcFirstCost = partial(calcNPV, firstCost=firstCost, income=income, cost=cost, implTime=implTime)
    minNPV, IRR = math.inf, None
    for disRate in range(1, 101):
        NPVirr = calcFirstCost(disRate=disRate / 100)[0]
    ...
    print("Некорректные входные данные для подсчета IRR")
